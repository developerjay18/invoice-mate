...
function LrPage({ params }: any) {
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      const response = await axios.delete(`/api/lrs`, {
        data: { lrId: id },
      });
  ...
  useEffect(() => {
    const fetchLrs = async () => {
      try {
        const response = await axios.post("/api/lrs/get-lrs", {
          companyId: id,
        });

        console.log(response.data);
        if (response.status === 200 && response.data.status === 200) {
          await setEntries(response.data.lrs);
          toast.success(response.data.message);
        } else {
          toast.error(response.data.error);
        }
        ...
        <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
          <div>
            <h2 className="text-xl font-semibold">
              Lrs -<span className="uppercase">{companyName} </span>{" "}
            </h2>
            <p className="mt-1 text-sm dark:text-slate-300 text-gray-700">
              All Lrs of {params.id}
            </p>
          </div>
          <div className="flex gap-x-4">
            <Button className="capitalize">
              <Link href={`/invoice/${company}/${id}/create`}>Add New LR</Link>
            </Button>
            <Button className="capitalize">
              <Link
